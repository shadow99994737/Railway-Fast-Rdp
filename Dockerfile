...
# God-level speed tuning: reduce network latency + maximize compression efficiency
# (no features disabled - only protocol/network-level performance settings)
RUN sed -i '/^\[Globals\]/a tcp_nodelay=true\ntcp_keepalive=true\nbulk_compression=true\nnew_cursors=true\nbitmap_cache=true\nbitmap_compression=true\nblock_input_on_new_connection=false' /etc/xrdp/xrdp.ini
